...
const callGemini = async (prompt: string): Promise<any> => {
    ...
    if (!ai) {
        ...
        return {
            summary: "API-ключ не предоставлен. Это пример ответа для генерации рекомендаций. ИИ анализирует данные со всех предыдущих шагов, чтобы предложить комплексные решения.",
            prioritized_recommendations: [
                {
                    segment_name: "Демо-шоссе (город)",
                    edge_id: 303,
                    priority: "Высокий",
                    problem_areas: ["Прогнозируемое падение скорости на 10 км/ч", "Высокий риск заторов в час пик"],
                    recommendations: ["Оптимизировать светофорное регулирование на пересечении с ул. Примерной", "Рассмотреть возможность введения выделенной полосы для общественного транспорта"]
                },
                {
                    segment_name: "Тестовый пр-т (город)",
                    edge_id: 202,
                    priority: "Средний",
                    problem_areas: ["Прогнозируемая перегрузка (120%)"],
                    recommendations: ["Провести аудит пропускной способности", "Улучшить информирование водителей о заторах через динамические табло"]
                }
            ]
